...
int *sorted_tasks;
double makespan,makespan_tb,makespan_c;
int deadline;
int critical;
double **computation_costs_tb;
...
void display_schedule()
{
    ...
    double temp=-9999.99;
    printf("\n\nSCHEDULE\n\n");
    printf("TASK :%d PROCESSOR :%d AST :%lf AFT :%lf\n",0+1,schedule[0].processor+1,schedule[0].AST,schedule[0].AFT);
    for(i=0; i<no_tasks; i++)
    {
    ...
    //////////////////////////////////////////////////
      for(i=0;i<no_tasks;i++)
    {
		if(temp<schedule[i].AFT)  
		temp=schedule[i].AFT;	
	}
    makespan=temp;
    if(makespan<=(double)deadline)
    {
    	printf("\nMakespan: %lf",makespan);
    	printf("\nBackup Tasks can be Assigned");
    	printf("\n----------------------------------------------------------------------------");
    	printf("\n----------------------------------------------------------------------------\n");
	...
	{
		printf("\nBackup Tasks cannot be Assigned");
		printf("\nMakespan: %lf",makespan);
...
///////////////////////////////////////////////////////////////   
            // Initialize Backup Computation Cost Table 
    computation_costs_tb=(double**)calloc(no_tasks,sizeof(double*));
    // Initialize each row for backup
    for(i=0; i<no_tasks; i++)
        computation_costs_tb[i]=(double*)calloc(no_machines,sizeof(double));
/////////////////////////////////////////////////////////////////
...
//-------------------------------------------------------------------------------------------------------
    ...
    // Read the acceptable Deasdline
    fscanf(fp,"%d",&deadline);
    
      // Read the Critical Task
    fscanf(fp,"%d",&critical);
    ...
    for(i=0; i<no_tasks; i++){
        for(j=0; j<no_machines; j++){
            computation_costs_tb[i][j]=computation_costs[i][j];
 	 	}
	}
	 ...
	 printf("\nCritical Taskksssssssssssssss\n");
    //////First for critical task
    float min1=8888.99;int t1,p1;
    for(j=0; j<no_machines; j++){
        		if(min1>computation_costs_tb[critical][j])
        		{
        			min1= computation_costs_tb[critical][j];
        			t1=critical;
        			p1=j;
				}
				
			}
			computation_costs[t1][p1]=9999.99;
				make_schedule();
    display_schedule_tb();
	//makespan_tb=makespan;
    ...
    printf("\nAll other Taskksssssssssssssss\n");
    //////Now for all tasks
   
        for(i=0; i<no_tasks ; i++){
		  float min1=9999.999;int t1,p1;
        	for(j=0; j<no_machines  ; j++){
        		if(min1>computation_costs[i][j])
        		{
        			min1= computation_costs[i][j];
        			t1=i;
        			p1=j;
				}
			...
			computation_costs[t1][p1]=9999.99;
			 if(makespan<=deadline){
						make_schedule();
    					display_schedule_tb();
    					printf("%f\t",computation_costs[i][j]);
    					//printf("Backup can be assigned to task %d",i);
					}
					else
					{
							printf("Backup cannot be assigned to task %d",i);
					}
	}
